...
#ifndef HASHMAP_TEXTURE_H
#define HASHMAP_TEXTURE_H
...
typedef void (*t_hash_texture_fn)(t_hasher *hasher, t_texture *key);
/// @typedef A function that drops a key-value pair
typedef void (*t_free_texture_fn)(t_kv_texture val);
/// @typedef A function that compares two keys and returns true if they are equal
typedef bool (*t_eq_texture_fn)(t_texture *lhs, t_texture *rhs);
...
typedef struct s_entry_texture
{
	t_usize						hash_id;
	t_kv_texture			kv;
	struct s_entry_texture *next;
} t_entry_texture;

/// @struct A hashmap of keys t_texture and values t_sprite
...
typedef struct s_hashmap_texture
{
	t_entry_texture **buckets;
	t_usize				  num_buckets;
	t_hasher			  hasher;
	t_hash_texture_fn hfunc;
...
t_hashmap_texture *hmap_texture_new(t_hash_texture_fn hash, t_eq_texture_fn	cmp, t_free_texture_fn free);
...
/// @return A new hashmap
t_hashmap_texture *hmap_texture_new_with_buckets(t_hash_texture_fn hash, t_eq_texture_fn cmp, t_free_texture_fn free, size_t cap);
...
void hmap_texture_clear(t_hashmap_texture *hmap);
...
bool hmap_texture_insert(t_hashmap_texture *hmap, t_texture key, t_sprite value);
...
t_entry_texture *hmap_texture_get_entry(t_hashmap_texture *hmap, t_usize hash, t_texture *key, t_entry_texture	**prev);
...
t_error hmap_texture_iter(t_hashmap_texture *self, t_error (*func)(t_usize idx, const t_texture *key, t_sprite *val, void *ctx), void *ctx);
...
/// @return An error code
t_error hmap_texture_clone(t_hashmap_texture *self, t_error (*clone)(const t_kv_texture *val, void *ctx, t_kv_texture *out), void *ctx, t_hashmap_texture **out);
